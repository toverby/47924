...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($_POST['first_name'])) {
		...
		$fn = $mysqli->mysqli_real_escape_string($database, trim($_POST['first_name']));
	}
	...
	if (empty($_POST['last_name'])) {
		...
		$ln = $mysqli->mysqli_real_escape_string($database, trim($_POST['last_name']));
	}
	...
	if (empty($_POST['email'])) {
		...
		$e = $mysqli->mysqli_real_escape_string($database, trim($_POST['email']));
	}
	...
	if (empty($errors)){ 
		...
		$q = "INSERT INTO  to2446992_New_Customer (first_name, last_name, email,registration_date) VALUES
		 ('$fn', '$ln', '$e', NOW() )";	
...
<title><?php echo $page_title; ?></title>
	...
	<form id="Form" action="cust_contact.php" method="post">
